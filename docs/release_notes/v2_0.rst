...
Debug, Debug List and Debug Drawer nodes were combined into a single node called Viewer. It automatically determine if the data need to be drawn in a box or inline, and returns information such as datatype and list length (depending on input datatype). The result can also be output to text block or console.
